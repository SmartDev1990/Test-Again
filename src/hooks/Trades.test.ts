...
import { Pair, TokenAmount, CurrencyAmount, Trade } from '@smartdev1990/dex-sdkv3'
...
describe('Trade', () => {
  describe('#useAllCommonPairs', () => {
    ...
    it('should filter only exist Pair', () => {
      mockUsePairs.mockReturnValue([
        [PairState.EXISTS, new Pair(new TokenAmount(briseToken.wbnb, '1'), new TokenAmount(briseToken.cake, '1'))],
        [PairState.INVALID, new Pair(new TokenAmount(briseToken.busd, '1'), new TokenAmount(briseToken.cake, '1'))],
        [PairState.LOADING, new Pair(new TokenAmount(briseToken.busd, '1'), new TokenAmount(briseToken.wbnb, '1'))],
        [PairState.EXISTS, null],
      ])

      const { result } = renderHook(() => {
        const pairs = Trades.useAllCommonPairs(briseToken.wbnb, briseToken.cake)
        return {
          pairs,
        }
      })
      ...
      expect(result.current.pairs).toStrictEqual([
        new Pair(new TokenAmount(briseToken.wbnb, '1'), new TokenAmount(briseToken.cake, '1')),
      ])
    })
    it('should filter out duplicated Pair', () => {
      mockUsePairs.mockReturnValue([
        [PairState.EXISTS, new Pair(new TokenAmount(briseToken.wbnb, '1'), new TokenAmount(briseToken.cake, '1'))],
        [PairState.EXISTS, new Pair(new TokenAmount(briseToken.wbnb, '1'), new TokenAmount(briseToken.cake, '1'))],
        [PairState.EXISTS, new Pair(new TokenAmount(briseToken.cake, '1'), new TokenAmount(briseToken.wbnb, '1'))],
        [PairState.EXISTS, null],
      ])
    ...
    it('should get all pair combinations wbnb, cake', () => {
      ...
      renderHook(() => {
        Trades.useAllCommonPairs(briseToken.wbnb, briseToken.cake)
      })
    ...
    it('should get all pair combinations, wbnb, wbnb', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(briseToken.wbnb, briseToken.wbnb)
      })
  ...
  describe('#useTradeExactIn/Out', () => {
    ...
    it('should call with maxHops 1 with singleHopOnly', () => {
      const allowPairs = [new Pair(new TokenAmount(briseToken.wbnb, '1'), new TokenAmount(briseToken.cake, '1'))]
      const argA = CurrencyAmount.ether('1000000')
      const argB = briseToken.cake
      renderHook(
        () => {
